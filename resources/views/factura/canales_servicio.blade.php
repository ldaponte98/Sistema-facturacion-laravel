                ...
                @foreach ($canales as $canal)
                    @if ($canal->id_dominio != App\Dominio::get('Mesa') and $canal->id_dominio != 49)
                        <li onclick="location.href = '{{ route('factura/facturador') }}?canal={{ $canal->id_dominio }}'">
                            <span class="fab-label"><b>Nueva venta por {{ $canal->nombre }}</b></span>
            ...
            <div class="card card-items">
                @if (session('id_perfil') != '5')
                    <div class="card-header">
                        <i class="fa fa-info-circle"></i><strong class="card-title pl-2">Tablero Principal</strong>
                    </div>
                    <div class="card-body">
                        <div class="tab-content" id="pills-tabContent">
                            ...
                            @foreach ($canales as $item)
                                <div class="tab-pane fade {{ $cont == 0 ? 'show active' : '' }}"
                                    id="nav-canal-{{ $item->id_dominio }}" role="tabpanel"
                                    aria-labelledby="nav-canal-{{ $item->id_dominio }}-tab">

                                    <div class="row">
                                        <div class="col-sm-12">
                                            <b><i>Ultima actualización: {{ date('Y-m-d H:i') }}</i></b> <span
                                                class="badge badge-primary ml-1"> <a href=""
                                                    title="Refrescar tiempos" class="ti-reload text-white"></a> </span>
                                        </div>
                                    </div><br>
                                    <div class="row">
                                        @if ($item->id_dominio == App\Dominio::get('Mesa'))
                                            ...
                                            @if (count($mesas) > 0)
                                                @foreach ($mesas as $mesa)
                                                    ...
                                                    @endphp
                                                    <div class="col-sm-2">
                                                        <div class="card pointer {{ $ocupada ? 'card-alert-info' : '' }}"
                                                            onclick="
                                                location.href = '{{ $ruta }}' ">
                                                            <div class="card-body">
                                                                <div class="mx-auto d-block">
                                                                    <img width="64" height="64"
                                                                        ...
                                                                        src="{{ $item->get_imagen() }}" alt="Mesa">
                                                                    <h5 class="text-center mt-2 mb-1">Mesa
                                                                        #{{ $mesa->numero }}</h5>
                                                                    @if ($ocupada)
                                                                        @if ($minutos > 0)
                                                                            <h4 class="text-center green">
                                                                                <b>{{ $minutos }} </b> <small>minutos
                                                                                    para entrega oportuna</small>
                                                                            </h4>
                                                                        @else
                                                ...
                                                <div class="col-sm-12">
                                                    <center>
                                                        <img width="350" height="350"
                                                            src="{{ asset('plantilla/images/empty_product.svg') }}">
                                                    ...
                                                    location.href = '{{ route('factura/facturador') . '?factura=' . $factura->id_factura }}' ">
                                                            <div class="card-body">
                                                                <div class="mx-auto d-block">
                                                                    <img width="64" height="64"
                                                                        class="mx-auto d-block"
                                                                        src="{{ $item->get_imagen() }}" alt="Pedido">
                                                                    <h5 class="text-center mt-2">
                                                                        {{ $factura->tercero->nombre_completo() }}</h5>
                                                                    <center>
                                                                        @php
                                                                        ...
                                                                        @endphp
                                                                        <small
                                                                            class="text-center bold-primary">{{ $fecha }}</small>
                                                                    </center>
                                                                </div>
                                                            </div>
                                                        </div>
                                                ...
                                                <div class="col-sm-12">
                                                    <center>
                                                        <img width="750" height="750"
                                                            src="{{ asset('plantilla/images/exito.png') }}"><br><br>
                                                    </center>
                                                </div>
                                            @endif
                                        @endif
                ...
                @else
                    <div class="card-header">
                        <i class="fa fa-exclamation-triangle"></i><strong class="card-title pl-2">¡Hola, eres el encargado
                            de ingresar los productos al sistema!</strong>
                    </div>
                    <div class="card-body">
                        <hr>
                        <div class="tab-content" id="pills-tabContent">
